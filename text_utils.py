...
class TextSource(object):
    ...
    def __init__(self, min_nchar, fn):
        ...
        with open(fn,'r') as f:
            self.txt = [word_tokenize(l.strip()) for l in f.readlines()]
        self.txt = [item for sublist in self.txt for item in sublist]
    ...
    def check_symb_frac(self, txt, f=0.35):
        ...
        def is_valid_char(ch):
            ...
            return cat.startswith('L') or cat.isdigit()  # Letters (incl. Khmer) or digits

        total = len(txt)
        if total == 0:
            return True  # or False, depending on how you want to treat empty input

        num_invalid = np.sum([not is_valid_char(ch) for ch in txt])
        frac = num_invalid / total
        return frac <= f
